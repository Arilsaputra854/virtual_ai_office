...
// Lokasi data bisa dipindah lewat env DATA_DIR (mis. untuk tes atau volume Docker).
export const DATA_DIR = process.env.DATA_DIR ? path.resolve(process.env.DATA_DIR) : REPO_DATA;
fs.mkdirSync(DATA_DIR, { recursive: true });
const CONFIG_PATH = path.join(DATA_DIR, 'office.json');
const EXAMPLE_PATH = path.join(REPO_DATA, 'office.example.json');
