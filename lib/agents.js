...
export async function runAgent({ agentId, messages, ctx }) {
  ...
  try {
    ...
    setStatus(agent.id, 'error', truncate(err.message, 120), ctx.taskId);
    throw err;
  } finally {
...
export async function runTask(t, cfg) {
  ...
  try {
    const text = await runAgent({
      ...
      ctx: { depth: t.depth, taskId: t.id, chain: [t.assignee], noDelegate: finalize },
    });
    afterRun(t.id, text);
    ...
    board.updateTask(t.id, { status: 'gagal', waiting: false, finishedAt: Date.now(), result: `Error: ${err.message}` });
    log(t.assignee, `gagal di #${t.id}: ${truncate(err.message, 120)}`, { taskId: t.id, error: true });
    notifyParent(board.getTask(t.id));
  } finally {
